...
const ChallengeItemsForm = ({
  ...
  const handleItemListDisplay = (() => {
    if (selectChallenge === undefined) return "미진행";
      ...
      if (selectChallenge.status === "SUCCESS") {return "성공"}
      else if (selectChallenge.status === "FAIL") {return "실패"}
      else if (selectChallenge.status === "PROGRESS") {return "진행중"}
      else {return "미할당"}
    }
  })
  ...
  <>
  {
    <div className="challenge_items_header">
      ...
      <div className="item_list_text_rightblock">
        <div className={`item_list_display ${selectChallenge && selectChallenge.status}`}>
